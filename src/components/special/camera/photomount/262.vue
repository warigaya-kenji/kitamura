<template>
  <v-app>
    <div class="photomount-262">
      <div class="main-contents-wrap">
        <!-- ↓ パンくず -->
        <breadcrumbs :breadcrumbs="breadcrumbs" />

        <!-- ↓ 特集IDでの商品一覧表示 -->
        <product-list-by-special-id class="mb-5" :specialId="262" />
      </div>
    </div>
  </v-app>
</template>
...
export default Vue.extend({
  ...
  setup: (props, context) => {
    document.title = 'チクマ V-67 差し込み式で簡単にできる写真台紙 | カメラのキタムラネットショップ';
    document
    ...
    const state = reactive({
      ...
      breadcrumbs: [
        ...
        {
          path: 'チクマ V-67 差し込み式で簡単にできる写真台紙',
          disabled: true
        }
      ]
    });
