...
export default Vue.extend({
  name: 'product-list-by-special-id',
  components: {
    product: Product,
  ...
  setup: (props, context) => {
     ...
     * 特集商品の取得
     */
    const getSpecialProduct = async (id: string) => {
      state.loaded.results = false;
      try {
        state.results = await SpecialProductService.getSaleSpecialProduct(id);
    ...
    watch(
      () => [props.specialId],
      () => {
        getSpecialProduct(`${props.specialId}`);
      },
      { immediate: true }
    );
